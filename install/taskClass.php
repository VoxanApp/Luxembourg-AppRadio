...
class Core {
    ...
    function write_config($data) {
        ...
        $output_path = '../includes/connection.php';

        $database_file = file_get_contents($template_path);

        $new = str_replace("%DB_HOST%", $data['hostname'], $database_file);
        $new = str_replace("%DB_USER%", $data['username'], $new);
        $new = str_replace("%DB_PASSWORD%", $data['password'], $new);
        $new = str_replace("%DB_NAME%", $data['database'], $new);
